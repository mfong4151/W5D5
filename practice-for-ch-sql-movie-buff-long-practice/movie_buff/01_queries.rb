...
def directed_by_one_of(them)
  ...
  #1
  #Movie.select(:id, :title).where('director_id IN (18, 360)')

  #2
  #Actor.select(:id).where("name = 'George Lucas' OR name = 'Steven Spielberg'")
  #Movie.select(:id, :title).joins(:actors).where('actors.name IN (?)',them)
  
  #3

  #Movie.select(:id, :title).where(director_id: Actor.select(name).where('name in (?)', them))
  Movie.joins(:director).where('director_id = ?')
