...
def what_was_that_one_with(those_actors)
  ...
  #Movie.joins(:actors).where('name ALL (?)',those_actors).select(:title, :id).distinct
  Movie.joins(:actors).group('movies.id').where("name IN (?)",those_actors).select(:title, :id)

  Movie.joins(:actors).group('movies.id').where("name IN (?)",  ['Ben Affleck', 'Matt Damon']).select(:title, :id)
  ...
  INNER JOIN actors ON actors.id = castings.actor_id
   WHERE (name IN ('Ben Affleck','Matt Damon')) GROUP BY movies.id
end
